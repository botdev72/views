...
/____/\__,_|_|_|      \_/ |_|\___| \_/\_/ |___/
    ...
    -----------------------------  
   | Developer : @ownedbyrain    |
   | Channel Telegram : @strbzul |
    -----------------------------''')
def tui():
    while True:
        print(logo)
        print(f'''{B}[ Data ]: {G}{channel.capitalize()}/{post}
{B}[ Total Views ]: {G}{real_views} ✅
 
{S}[ Koneksi Error ]: {E}{proxy_errors} 🚫
{S}[ Token Error ]: {E}{token_errors} ❌
 
{G}[ Kecepatan ]: {B}{active_count()} ms ''')
        sleep(2);system('cls' if name == 'nt' else 'clear')

system('cls' if name == 'nt' else 'clear')
print(logo)
channel, post = input(f"\n{G}[+] Link nya boss -> {S}").replace('https://t.me/', '').split('/')
...
try:
    ...
    print(f'{G}[√]Memulai.. | Tunggu Bentar...')
